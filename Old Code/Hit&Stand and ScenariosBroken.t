...
Font.Draw ("Dealer/PC:", 100, maxy div 2 + 50, font3, yellow)
Font.Draw ("You:", 100, maxy div 2 - 80, font3, yellow)
...
loop
	...
	if player.total > 21 then
	    put "BUST"
	elsif player.total < computer.total then
		...
		if Mx > 245 and Mx < 300 and My > 30 and My < 60 and button = 1 then
		    ...
		    put "Your new total: ", player.total
		    if player.total > computer.total then
			draw (computer)
			if computer.total > 21 then
			    put "Computer drew and busted u win"
			else
			    put "You win"
			end if
		    end if
	...
	elsif computer.total <= 16 or computer.total < player.total then
	    delay (500)
	    draw (computer)
	    put "Computer drew, new total of PC is: ", computer.total
	    if computer.total = player.total then
		put "Tie. Game pushed"
	    elsif computer.total = 21 then
		put "Computer drew Blackjack & won"
	    end if
	    if computer.total > 21 then
		...
		put "Computer drew Blackjack, PC wins"
	    end if
	end if
	if player.total > 21 then
	    put "You busted! Computer wins"
	...
	if computer.total < player.total then
	    draw (computer)
	    put "PC drew. New total: ", computer.total
	    if computer.total > 21 then
		put "Computer busted. You win"
	    elsif computer.total > player.total then
		put "PC Wins"
	    else
		put "YOU WIN!"
	    end if
		...
		Mx > 430 and Mx < 483 + 30 and My > 30 and My < 60 and button = 1 and computer.total > player.total and computer.total < 21 then

	    delay (200)
	    put "Computer wins"
	    exit

	elsif Mx > 430 and Mx < 483 + 30 and My > 30 and My < 60 and button = 1 and computer.total = player.total and computer.total < 20 then
	    draw (computer)
	    if computer.total > player.total then
		delay (500)
		put "Computer decided to hit"

		put "Computer's new total: ", computer.total
		if computer.total = 21 then
		    put "Computer drew blackjack, PC wins"
		elsif computer.total < player.total then
		    put "You win"

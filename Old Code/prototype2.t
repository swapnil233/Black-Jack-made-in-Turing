...
procedure mouseOver
	...
	if Mx > 340 and Mx < 420 and My > 70 and My < 170 then
	    Pic.Draw (picDeck (player.cardSuit (2), player.cardFace (2)), maxx div 2 + 30, 80, 0)
	else
...
procedure betmoney
    loop
	put "How much do you want to bet? You have, $", mymoney, " and Dealer has $", pcmoney
	get bet
	if bet > mymoney then
...
procedure game
	...
	if Mx > 245 and Mx < 300 and My > 30 and My < 60 and button = 1 then
	    ...
	    Draw.Text (intstr (player.total), 114, 118, font3, yellow)
	    %put "New Total: ", player.total, " | " ..
	elsif Mx > 430 and Mx < 483 + 30 and My > 30 and My < 60 and button = 1 then
	    Draw.FillBox (114, 118, 140, 140, green)
	    Draw.Text (intstr (player.total), 114, 118, font3, black)     %final
	    %put "Final total: ", player.total
	    exit
	end if
	delay (100)
    end loop

    if computer.total <= 17 then     %Just deals PC a card if its = or < 17. Keeps dealing unitil PC Total is > 17
	delay (1000)
	loop
	    delay (1000)
	    draw (computer)
	    ...
	    %put "Computer Final Total: ", computer.total
	end loop
    end if
    ...
    %STILL IN WORK%
    if computer.total = 17 and player.total > computer.total then
	delay (1000)
	put "YOU WIN"
	mymoney := mymoney + bet
	pcmoney := pcmoney - bet
	put "Your wallet: ", mymoney
	put "Dealer Wallet: $", pcmoney
